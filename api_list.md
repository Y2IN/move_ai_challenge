...
- **✅ 완료 (39):** 랜딩·대시보드 · 화물등록 · 매칭 · 조율(#21~#26) · 편익 · 보조금 ·
  ESG · 마스터 · 정산. **#36 문단 재생성은 구현+배선 완료**(경로는
  `POST /api/subsidy/applications/{id}/paragraphs/{key}`), **#38 변경 이력 배선 완료**.
- **➖ MVP 제외 (5):** 인증 #1~#5 — 계정 없이 데모(역할 선택은 클라 처리)
- **❌ 미구현 (2):** #30 preflight(정적 체크리스트+#40 실측으로 대체) ·
  #43 코레일 승인
...
|---|---|---|---|---|---|
| 10 | ✅ | POST | `/api/freights/parse` | **자연어 문장 → 구조화 폼.** GET=케이스 목록, POST={text\|caseId} | 🔶 데모(LLM 미연결, 케이스 반환) |
