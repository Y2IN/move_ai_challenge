...
- **✅ 완료 (9):** #7 대시보드 · #8 매칭목록 · #11 등록 · #13 목록 · #14 수정 · #15 삭제 · #16 매칭요청 · #18 공차현황 · #19 편성확정
...
- **⛔ 보류 (2):** #9 상세(#8 인라인으로 대체) · #17 job(매칭이 sync라 불필요)
...
| 8 | ✅ | GET | `/api/matches?persona=&status=&page=` | AI 합적 매칭 현황 목록 | `route, wagon, sub, tons, loadRate, status(done\|group\|wait), saving`. detail 인라인 |
| 9 | ⛔ | GET | `/api/matches/{id}` | 행 펼침 상세 | #8 detail 인라인으로 대체 — 미구현(의도) |
