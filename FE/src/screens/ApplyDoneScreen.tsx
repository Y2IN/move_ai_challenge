    ...
    <div className="flex flex-col gap-3">
      ...
      {doc.inputOrigin === "fixture" && (
        <div className="rounded-[10px] border border-[#F2B33D] bg-[#FFF8EB] px-4 py-3 text-[13px] leading-[1.7] text-[#A96A00]">
          <b>고정 예시 수치입니다.</b> 실제 편성·계산에서 값을 만들지 못해
          시연용 고정값으로 채웠습니다
          {doc.inputNote ? ` — ${doc.inputNote}` : ""}. 이 상태로 제출하면 안
          됩니다.
        </div>
      )}
      ...
      {doc.keptUserEdits.length > 0 && (
        <div className="rounded-[10px] border border-[#D6E7FF] bg-[#F5F9FF] px-4 py-3 text-[13px] text-[#1B64DA]">
          직접 편집한 문단 {doc.keptUserEdits.length}개는 덮어쓰지 않고 그대로
          두었습니다.
        </div>
      )}
...
function PanelCard({
  ...
  card?: { label: string; value: string; adopted: boolean };
}) {
    ...
    <div
      className={[
        ...
        adopted
          ...
          : "border-[#F2F4F6] bg-[#F9FAFB]",
      ].join(" ")}>
      <div
        className={`text-[13px] font-bold ${adopted ? "text-[#1B64DA]" : "text-[#8B95A1]"}`}>
        {card?.label ?? "—"}
      </div>
      <div
...
export function ApplyDoneScreen({
  ...
  const docBusy =
    (!doc.doc && !doc.error) || doc.regenerating || doc.busyKeys.length > 0;
      ...
      <section className="grid grid-cols-[1fr_380px] items-start gap-5">
        ...
        <aside className="sticky top-6 flex flex-col gap-4">
          <div className="rounded-[20px] bg-white p-6">
            <span className="text-base font-extrabold tracking-[-0.02em] text-[#191F28]">
              보조금 산정
            </span>
            ...
            <div className="mt-[18px] border-t border-[#F2F4F6] pt-[18px]">
              <div className="text-sm font-semibold text-[#6B7684]">
                {panel?.resultLabel ?? "보조금 산정 결과"}
              </div>
              <div className="mt-2 text-[34px] font-extrabold tracking-[-0.04em] text-[#191F28]">
                {panel?.result ?? "—"}
              </div>
              <div className="mt-1.5 text-sm tabular-nums text-[#8B95A1]">
                {panel?.resultKrw ?? ""}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2.5 rounded-[20px] bg-white p-6">
            ...
            {panel && (
              <p className="mt-1.5 text-[13px] leading-relaxed text-[#8B95A1]">
                {panel.caution}
              </p>
            )}
          </div>
