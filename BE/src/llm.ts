...
/**
 * 기본 모델. **키가 어느 등급인지 모르는 상태에서도 응답하는 쪽**을 기본값으로 둡니다.
 *
 * 배포 환경은 `GEMINI_MODEL` 로 덮어씁니다 (현재 프로덕션은 `gemini-3.6-flash`).
 * 여기 기본값은 환경변수가 없을 때만 쓰이므로, 새로 클론한 사람이 무료 키로도
 * 바로 돌려볼 수 있는 값이어야 합니다.
 *
 * 모델을 고를 때 걸리는 지뢰 두 개 — 둘 다 실제로 밟았습니다:
 *
 *   `gemini-3.6-flash`  무료 티어 일일 한도가 **모델당 20회**라 제일 먼저 마릅니다.
 *                       마르면 429 로 전 문단이 폴백 초안이 됩니다. 유료(Tier 1+)
 *                       키에서는 문제 없습니다.
 *   `gemini-2.5-flash`  신규 사용자에게는 404 (제공 중단). 그런데 **모델 목록 API
 *                       에는 여전히 보입니다** — 목록만 보고 고르면 걸립니다.
 *
 * `-lite` 계열은 할당량이 따로·넉넉해 무료 키로도 계속 응답합니다. 문단이 3~5문장으로
 * 짧고 숫자는 환각 검출기가 따로 막으므로 기본값으로 쓰기에 충분합니다.
 */
