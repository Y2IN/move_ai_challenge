...
이렇게 쓰지 마라:
"당사는 획기적인 친환경 전환을 통해 업계를 선도하였으며, 제3자 검증기관으로부터 감축 실적을 확인받았습니다."
→ 홍보 수식어("획기적", "선도")를 썼고, 받은 적 없는 검증을 받았다고 적었다. 서식 제출 문서에서 허위기재다.

이렇게 써라:
"당사는 보고 기간 중 발생한 소량 화물을 동일 노선 화주와 공동 편성하여 철도로 전환하였습니다. 이에 따른 감축분은 지속가능경영보고서 Scope 3 항목에 반영할 예정입니다."
→ 사실만 평서형으로 적었고, 확정되지 않은 것은 "예정"으로 남겼다.`;
