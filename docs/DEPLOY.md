...
| `GEMINI_MODEL` | ✅ `gemini-3.1-flash-lite` | 비우면 코드 기본값도 같다. **`gemini-3.6-flash` 를 쓰지 말 것** — 무료 티어 일일 할당량이 제일 먼저 말라 전 문단이 429 폴백으로 떨어진다. `gemini-2.5-flash` 는 신규 사용자에게 404 (모델 목록에는 아직 보인다) |
